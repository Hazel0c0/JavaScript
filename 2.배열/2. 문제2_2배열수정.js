...
var changeMember, index, newMember;

// group.push();

while (group.length!==0) {
  changeMember = prompt(`현재 멤버:[${group}]\n변경할 멤버를 입력하세요.`);

  if (group.includes(changeMember)) {
    newMember=prompt(`새로운 멤버 이름을 적어주세요`);
    index=group.indexOf(changeMember);
    group.splice(index,1,newMember);
    alert(`수정 완료!\n현재 멤버:[${group}]`);
  } else {
